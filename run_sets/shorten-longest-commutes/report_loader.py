...
shorten-longest-commutes's summary.qmd and slides.qmd.
...
Mirrors bring-work-trips-closer-to-home/report_loader.py's structure and
most of its build_* functions verbatim (same TDM version/network, same
curated output shapes) -- but this run set has only one dimension
(shift_pct: 5%/10%, region-wide), not that run set's geography_label x
shift_pct factorial, so SCENARIO_META has no geography_label column, and
everything geography/city-specific there (GEO_ORDER, TARGET_CITIES,
compute_city_area_shift_trips, build_city_results) has no counterpart here.
compute_region_wide_shift_trips() is this run set's analogous "scale of
mechanism" number, computed by re-running the actual redistribution
algorithm's own selection logic (scripts/redistribute_longest_hbw_trips.py)
against curated baseline data, rather than reading the model run's own
console output (which isn't captured anywhere committed -- see CLAUDE.md).

load() discovers which scenarios actually have a curated run and builds
every table from just those -- so this module works unmodified if more
shift levels are ever added; nothing here needs to change beyond adding a
...
import redistribute_longest_hbw_trips as algo  # noqa: E402

RUN_SET_ID = "shorten-longest-commutes"

# scenario_id -> shift level, per run_set.yaml's scenario design (a single
# no-shift baseline plus 5%/10% region-wide longest-trip redistribution).
SCENARIO_META = pd.DataFrame([
    {"scenario_id": "Shorten00", "shift_pct":  0},
    {"scenario_id": "Shorten01", "shift_pct":  5},
    {"scenario_id": "Shorten02", "shift_pct": 10},
])
BASELINE_SCENARIO = "Shorten00"
...
# entries keep their original name. Identical to bring-work-trips-closer-to
# -home's suffixes -- same run_set.yaml outputs.include, same TDM version.
...
# scripts/redistribute_longest_hbw_trips.py's own SKIM_NOACCESS_SENTINEL,
# excluded from the trip-length distribution and the region-wide selection
# re-derivation so a handful of technically-unconnected cells can't
# masquerade as extreme-long trips.
...
# Manual corridor orientation/freeway classification -- identical to
# bring-work-trips-closer-to-home/report_loader.py's own (same network,
# same TDM version); see that module's docstrings for the full rationale.
...
def _curated_path(scenario_id: str, suffix: str) -> str:
    ...
    if run is None:
        raise FileNotFoundError(
            ...
            "(or import-manual-run for Shorten00) first."
        )
...
def compute_region_wide_shift_trips(shift_pct: float = 10) -> dict:
    """Reproduces scripts/redistribute_longest_hbw_trips.py's own
    region-wide selection (select_region_wide/weighted_median) against the
    curated baseline HBW trip matrix (Daily = Peak+Off-Peak) and Peak
    distance skim -- a report-side "scale of mechanism" check, not a read
    of the actual run's own console output (the redistribution script's
    print()s go to Cube's own console window, not to anything committed --
    see CLAUDE.md).

    Uses the curated Peak-period distance skim (average of the AM/PM final
    assignment skims' GP_Dist) as a stand-in for the real redistribution
    step's actual basis (skm_auto_Pk.mtx's dist_GP, a pre-mode-choice
    highway skim that isn't curated into runs/ -- only the post-assignment
    final skims are). Both should be close: GP_Dist is a largely static
    network-distance attribute along whichever path was assigned, not
    something that moves much between the pre-mode-choice and
    post-assignment network states (same simplification
    bring-work-trips-closer-to-home/report_loader.py's own distance-skim
    docstring already documents, confirmed there to differ by well under
    1% across periods). Treat this as an order-of-magnitude check that the
    algorithm behaved sensibly on real data, not an exact replay of what
    happened inside the model run.
    """
    ...
    _, selection = algo.select_region_wide(combined, distance, shift_pct / 100)

    valid = (combined > 0) & (distance < SKIM_NOACCESS_SENTINEL)
    median_distance = algo.weighted_median(distance[valid], combined[valid])

    return {
        "total_region_trips": selection["total_region_trips"],
        "target_moved_trips": selection["target_moved_trips"],
        "actual_moved_trips": selection["actual_moved_trips"],
        "pct_of_total": selection["actual_moved_trips"] / selection["total_region_trips"] * 100,
        "cells_selected": selection["cells_selected"],
        "median_distance": median_distance,
        "band_miles": algo.DISTANCE_BAND_MILES,
        "shift_pct": shift_pct,
    }
...
def build_hbw_trip_length_distribution(scenario_ids: list) -> dict:
    ...
    set's mechanism (trim the longest region-wide trips, redistribute
    toward the regional median) actually did what it was built to do."""
