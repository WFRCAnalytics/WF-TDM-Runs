"""report_snapshot_script for shorten-longest-commutes (see run_set.yaml).
Invoked by `tdmruns snapshot-run-set --run-set shorten-longest-commutes` as:
...
bring-work-trips-closer-to-home / shorten-lengthen-all-work-trips). See
that module's own docstring for exactly what's frozen, why Daily isn't
stored separately, and why the arrays are compressed float32 rather than
bare np.save. Must be run while runs/shorten-longest-commutes still has
