;System
    ...
    *(ECHO 'model crashed' > redistribute_longest_hbw_trips.txt)
...
;====================================================================================================================================================================;
;Python HBW Longest-Trip Redistribution (shorten-longest-commutes)
;====================================================================================================================================================================;

; redistribute the longest hbw_trip_redistribution_portion share of
; region-wide HBW trip volume (by distance, ranked across all OD pairs, not
; per origin) toward destinations near the regional median commute distance
; -- hbw_trip_redistribution_portion is read directly by the python script
; from the scenario's own ShortenXX.yaml `variables:` block (not a Control
; Center key, so Cube itself never sees it; only
...
**"@ModelDir@\2_ModelScripts\_Python\py-tdm-env\python.exe" "@ModelDir@\..\run_sets\shorten-longest-commutes\scripts\redistribute_longest_hbw_trips.py" --run-set-dir "@ModelDir@\..\run_sets\shorten-longest-commutes" --scenario-id "@ScenarioName@" --numveh-mtx "@ScenarioDir@\Temp\4_ModeChoice\pa_HBW_NumVeh_noXI.mtx" --destchoice-mtx "@ScenarioDir@\Temp\4_ModeChoice\pa_AllPurp.2.DestChoice.mtx" --skim-mtx "@ScenarioDir@\4_ModeChoice\1a_Skims\skm_auto_Pk.mtx" 1>&2
...
*(rmdir /s /q "@ModelDir@\..\run_sets\shorten-longest-commutes\scripts\__pycache__")
...
RUN PGM=MATRIX
    ...
    PRINT FILE='@ScenarioDir@\_Log\_RunTime.txt',
        APPEND=T,
        LIST='\n    Longest HBW Trip Redistribution     ', formatdatetime(@ScriptStartTime@, 40, 0, 'yyyy-mm-dd,  hh:nn:ss'),
...
*(del redistribute_longest_hbw_trips.txt)
