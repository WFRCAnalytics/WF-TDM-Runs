...
require([
  ...
  async function loadAndDisplaySplash(disclaimer) {
    ...
    if (disclaimer.on) {
      ...
      modalContent.innerHTML = '<span id="closeModalX" class="modal-close">&times;</span>';
  ...
  async function initVizMapListeners() {
    ...
    document
      .getElementById("selectCompareType")
      .addEventListener("calciteSelectChange", updateActiveVizMap);
    document
      .getElementById("comparisonScenario")
      .addEventListener("calciteBlockToggle", updateActiveVizMap);
...
function getFirstScenarioWithTrendData(jsonName) {
  ...
  return dataScenarios.find((scenario) =>
    scenario.jsonData.hasOwnProperty(jsonName)
  );
}
