...
def test_latest_successful_run_skips_newer_failure(tmp_path):
    ...
    source.mkdir()
    _write_run(tmp_path, "rs", "Closer00", "20260101-000000-aaaa", "success", source)
    _write_run(tmp_path, "rs", "Closer00", "20260102-000000-bbbb", "failed", source)

    result = md.latest_successful_run(tmp_path, "rs", "Closer00")
...
def test_latest_successful_run_none_when_no_success(tmp_path):
    source = tmp_path / "source"
    source.mkdir()
    _write_run(tmp_path, "rs", "Closer00", "20260101-000000-aaaa", "failed", source)

    assert md.latest_successful_run(tmp_path, "rs", "Closer00") is None
...
def test_seed_copies_from_latest_successful_run_despite_newer_failure(tmp_path):
    ...
    dest = tmp_path / "dest"
    dest.mkdir()
    result = seed.seed(tmp_path, "rs", {"start_from_copy": "Closer00"}, dest)

    assert result == {"scenario_id": "Closer00", "run_id": "20260101-000000-aaaa"}
...
def test_seed_raises_when_no_successful_run(tmp_path):
    ...
    with pytest.raises(ScenarioSeedError, match="no successful recorded run"):
        seed.seed(tmp_path, "rs", {"start_from_copy": "Closer00"}, dest)
...
def test_seed_raises_when_source_folder_missing(tmp_path):
    ...
    _write_run(tmp_path, "rs", "Closer00", "20260101-000000-aaaa", "success", missing_source)
...
def test_lock_down_copy_suppresses_copy(tmp_path):
    ...
    result = seed.seed(
        tmp_path, "rs", {"start_from_copy": "Closer00", "lock_down_copy": True}, dest
    )
