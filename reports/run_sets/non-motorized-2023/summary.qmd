...
# Test scenarios 1-13 read whatever tdmruns import-manual-run(-set) most
# recently curated under runs/, resolved via report_data.py -- the same
# runs/-reading module the generic run-set pages use.
REPO_ROOT_REL = os.path.join('..', '..', '..')
sys.path.insert(0, os.path.join('..', '..'))
import report_data as rd

RUN_SET_ID = 'non-motorized-2023'
TDM_ROOT = os.path.join(REPO_ROOT_REL, 'tdm')
TDM_BASELINE_DIR = os.path.join(TDM_ROOT, 'Scenarios', RUN_SET_ID)
latest_runs = {r['scenario_id']: r for r in rd.latest_run_per_scenario(RUN_SET_ID)}
...
def curated_path(scenario_id, suffix):
    ...
    run = latest_runs.get(scenario_id)
    if run is None:
    ...
    return os.path.join(REPO_ROOT_REL, matches[0])
...
def load_scenario(test_id):
    if test_id == 0:
        ...
        path = curated_path(f'S{test_id:02d}', '_ZoneSummary_TripsByMode.csv')
    df = pd.read_csv(path)
...
t10_se = gpd.read_file(curated_path('S10', '.dbf'))
t11_se = gpd.read_file(curated_path('S11', '.dbf'))
