...
copy time). Every selected file is written regardless of size, but any file
whose actual written size exceeds the configured max_file_size_mb is marked
"committed": False and listed in that outputs/ folder's own auto-generated
.gitignore -- kept on disk locally (e.g. so reports still render fully on
the machine that curated it) but never committed to the repo, rather than
aborting curation and marking an otherwise-successful run "failed" over one
oversized file. This is what actually enforces the "no committed file over
the size ceiling" rule in practice, automatically, regardless of what
outputs.include declares.
...
plain datafile/column entries never do. The size ceiling is checked against
the bytes actually written -- the filtered/extracted/exported size for these
entries, the source size for a plain copy -- not the raw source size in all
cases, since the raw size of a to-be-transformed file says nothing about
...
def _write_outputs_gitignore(dest_dir: Path, uncommitted_names: list):
    ...
    if not uncommitted_names:
        ...
        return
    lines = [
        "# Auto-generated by tdmruns' output curation (src/tdmruns/outputs.py).",
...
def _manifest_entry(entry: dict, dst: Path, repo_root: Path, max_bytes: int) -> dict:
...
def curate(
    ...
    treated as a failure here rather than passed through unexamined. A
    selected file that turns out to be oversized once written does *not*
    fail the run on its own -- see copy_selected()'s "committed" handling.
    """
